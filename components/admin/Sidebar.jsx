...
import { LayoutDashboard, Package, ShoppingBag, Clock, Settings, ExternalLink, LogOut, X, Menu } from 'lucide-react';
    ...
    {NAV.map(item => {
      ...
      return (
        <Link key={item.href} href={item.href} onClick={onNavigate} className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${active ? 'bg-teal-600 text-white shadow-lift' : 'text-slate-700 hover:bg-slate-100'}`}>
          <item.icon className={`w-4.5 h-4.5 ${active ? 'text-white' : 'text-slate-500 group-hover:text-slate-700'}`} />
...
const SidebarContent = ({ pathname, onNavigate, router }) => (
  <>
    ...
    <div className="p-3 border-t border-slate-200 space-y-1">
      ...
      <button onClick={() => { localStorage.removeItem('cs_admin'); router.push('/'); }} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-rose-600 hover:bg-rose-50">
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex flex-col fixed inset-y-0 left-0 w-64 bg-white border-r border-slate-200 z-30">
        <SidebarContent pathname={pathname} onNavigate={() => {}} router={router} />
      </aside>
      ...
      {open && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div className="absolute inset-y-0 left-0 w-72 bg-white flex flex-col shadow-2xl">
            ...
            <SidebarContent pathname={pathname} onNavigate={() => setOpen(false)} router={router} />
          </div>
        </div>
      )}
    </>
