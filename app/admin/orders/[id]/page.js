...
import { ChevronLeft, Phone, MapPin, Calendar, CreditCard, CheckCircle2, Truck, Package, Home, Clock } from 'lucide-react';
...
const OrderDetail = () => {
  ...
  const load = () => fetch(`/api/orders/${id}`).then(r => r.json()).then(d => setOrder(d.order));
  useEffect(() => { load(); fetch('/api/slots').then(r => r.json()).then(d => setSlots(d.slots || [])); }, [id]);
