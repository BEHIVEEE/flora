...
import { ArrowRight, ShieldCheck, Truck, Stethoscope, Pill, HeartPulse, Bandage, Baby, Leaf, Sparkles, Activity, Star, Clock, Award, MessageCircle, Upload } from 'lucide-react';
import ProductCard from '@/components/ProductCard';
import ProductSkeleton from '@/components/ProductSkeleton';
import { Button } from '@/components/ui/button';
import { CATEGORIES } from '@/lib/seed-data';

const ICONS = { Pill, Stethoscope, HeartPulse, Bandage, Baby, Leaf, Sparkles, Activity };
        ...
        <div className="container max-w-7xl mx-auto px-4 py-10 md:py-16 grid md:grid-cols-2 gap-8 items-center relative">
          <div>
            <div className="inline-flex items-center gap-2 bg-teal-100 text-teal-800 text-xs font-semibold px-3 py-1.5 rounded-full mb-5">
              ...
              Apka Apna Chemist · Trusted by 1L+ families
            </div>
      ...
      <section className="container max-w-7xl mx-auto px-4 py-12 md:py-16">
        ...
        <div className="grid grid-cols-4 md:grid-cols-8 gap-3 md:gap-4">
          {CATEGORIES.map(c => {
            ...
            return (
              <Link key={c.id} href={`/products?category=${c.id}`} className="group bg-white rounded-2xl border border-slate-200 p-3 md:p-4 hover:shadow-lift hover:border-teal-200 transition-all text-center">
                <div className={`w-12 h-12 md:w-14 md:h-14 mx-auto rounded-xl bg-gradient-to-br ${c.color} text-white flex items-center justify-center mb-2 md:mb-3 group-hover:scale-110 transition-transform shadow-lift`}>
