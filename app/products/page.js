        ...
        <div className="grid lg:grid-cols-[240px_1fr] gap-6">
          ...
          <aside className="hidden lg:block">
            <div className="bg-white rounded-2xl border border-slate-200 p-4 sticky top-32">
              ...
              {CATEGORIES.map(c => (
                <button key={c.id} onClick={() => setCat(c.id)} className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium mb-1 ${category === c.id ? 'bg-teal-50 text-teal-800' : 'hover:bg-slate-50 text-slate-700'}`}>{c.name}</button>
              ))}
            </div>
          </aside>
            ...
            <div className="flex items-center justify-between mb-4 gap-3">
              <Sheet>
                ...
                <SheetContent side="left" className="w-72">
                  ...
                  {CATEGORIES.map(c => (
                    <button key={c.id} onClick={() => setCat(c.id)} className={`w-full text-left px-3 py-2.5 rounded-lg text-sm font-medium mb-1 ${category === c.id ? 'bg-teal-50 text-teal-800' : 'hover:bg-slate-50'}`}>{c.name}</button>
                  ))}
                </SheetContent>
              </Sheet>
