    ...
    if (renflairKey) {
      try {
        ...
        if (smsRes.ok) {
          ...
          // Persist plain OTP and method for short-lived verification
          await db.collection('otp_sessions').updateOne(
            { phone: phoneClean },
            { $set: { otp: String(otp), method: 'renflair' } }
          );
        } else {
...
export async function handleVerifyOTP(req, db, json, rateLimit, getClientIp, body = null) {
  ...
  try {
    ...
    } else if (session.otp && String(session.otp) === String(otp)) {
      // Renflair or direct OTP comparison (plain text)
      verified = true;
