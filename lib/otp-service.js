 ...
 */
export async function sendOTPViaMSG91(phoneNumber, otp) {
  try {
    const authKey = process.env.MSG91_AUTH_KEY;
    const templateId = process.env.MSG91_TEMPLATE_ID;

    if (!authKey) throw new Error('MSG91_AUTH_KEY not configured');
    if (!templateId) throw new Error('MSG91_TEMPLATE_ID not configured');
    ...
    url.searchParams.set('mobile', `91${phoneClean}`);
