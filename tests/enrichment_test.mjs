...
function runTests() {
  ...
  const targetMed = { product_name: "Ceftum 250mg Tablet Extra", manufacturer: "Glaxo", pack_size: "10" };
  const scoreMed = computeMatchScore(sourceMed, targetMed, false, 0.20); // 0.20 Fuse score = 80% similarity
  ...
  // 3. Pharmacy safety rule: Strength Mismatch (250mg vs 500mg) should be rejected
  const sourceMed2 = { product_name: "Ceftum 250mg", manufacturer: "Glaxo", pack_size: "10" };
  const targetMed2 = { product_name: "Ceftum 500mg Tablet", manufacturer: "Glaxo", pack_size: "10" };
  const scoreMed2 = computeMatchScore(sourceMed2, targetMed2, false, 0.15); // 85% base similarity
  
  assert(
    scoreMed2.confidence < 80.0 && scoreMed2.reviewStatus === 'rejected',
    `Strength mismatch (250mg vs 500mg) must decrease confidence and be rejected (confidence: ${scoreMed2.confidence}%)`
  );

  // 4. Low similarity (should reject < 80%)
